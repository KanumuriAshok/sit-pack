...
class Geoserver:
    ...
    def publish_featurestore(self, store_name: str, pg_table: str,srs_data:str, workspace: Optional[str] = None):
        """
        ...
        layer_xml = "<featureType><name>{}</name><srs>{}</srs><projectionPolicy>FORCE_DECLARED</projectionPolicy></featureType>".format(pg_table,srs_data)
        headers = {"content-type": "text/xml"}
    ...
    def publish_style(
        ...
        style_xml = (
            "<layer><defaultStyle><name>{}</name><srs>{}</srs><projectionPolicy>FORCE_DECLARED</projectionPolicy></defaultStyle></layer>".format(style_name, srs_name))
